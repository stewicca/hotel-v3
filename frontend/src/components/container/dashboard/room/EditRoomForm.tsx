...
export default function EditRoomForm({ room }: { room: any }) {
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    await axios.put(`http://localhost:8080/room/${room.id}`, data, { headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }})
      .then(function (response) {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className={cn('flex flex-col gap-4')}>
        <h1 className={cn('text-2xl font-semibold')}>Edit Room</h1>
        <FormField
